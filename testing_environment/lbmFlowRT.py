...
solid = np.fromfunction(lambda x, y: (x-cx)**2+(y-cy) **
                        2 < r**2, (nx, ny)).astype(np.bool)
nulb = uLB*r/Re
...
feq = equilibrium(np.ones((nx, ny)), vel)
fin = feq.copy()
...
def display():
    ...
    # L  #Color code of white
    plot_rgba[solid.T.flatten() == True] = 4294967295

    # Fill the pixel buffer with the plot_rgba array
    glBufferData(GL_PIXEL_UNPACK_BUFFER, plot_rgba.nbytes,
                 plot_rgba, GL_STREAM_COPY)

    # Copy the pixel buffer to the texture, ready to display
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, ni, nj,
...
def run_opengl():
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, ni, nj,
...
def get_cmap_from_matplotlib(cmap=cm.jet):
    ...
    for i in range(ncol-1):
        # Not sure why this is inverted, I was expecting r, g, b.
        b, g, r, _ = cmap(i)
